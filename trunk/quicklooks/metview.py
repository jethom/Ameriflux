...
#yesterdaydir = datadir + yesterday.strftime('%Y%m%d')
#if os.path.exists(yesterdaydir):
     #files1 = filter(os.path.isfile, glob(yesterdaydir + "/*met*"))
     #files1.sort(key=lambda x: os.path.getmtime(x))
#todaydir = datadir + currenttime.strftime('%Y%m%d')
#if os.path.exists(todaydir):
     #files2 = filter(os.path.isfile, glob(todaydir + "/*met*"))
     #files2.sort(key=lambda x: os.path.getmtime(x))
#metfiles = files1 + files2
# if no files exist exit
if len(metfiles)==0:
    sys.exit("No files exist for plotting")
...
ax1.set_ylabel('water level', color = 'b')
