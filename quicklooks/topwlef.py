#!/usr/bin/env python
#
# script to make quicklook plots of the top data files from WLEF
#
...
#-----------------------------------------------------------
datadir = "/data/incoming/WLEFFlux/FluxData/"
...
if os.path.exists(figdir + '/temptop.png'):
    os.remove(figdir + '/temptop.png')
...
yesterdaydir = datadir + yesterday.strftime('%Y_%m_%d_*/') + logger + '/'
todaydir = datadir + currenttime.strftime('%Y_%m_%d_*/') + logger + '/'
...
axarr[0].set_title('Top wind speed ' + figdate)
axarr[0].grid(True)
...
axarr[0].set_title('Top Flow rates ' + figdate)
axarr[0].grid(True)
...
ax.set_xlabel('Time')
ax.set_ylabel('%')
plt.title('Top Relative Humidity ' + figdate)
ax.xaxis.set_major_locator(hrs3)
ax.xaxis.set_major_formatter(hrsfmt)
ax.autoscale_view()
ax.grid(True)
...
ax.set_xlabel('Time')
ax.set_ylabel('degC')
plt.title('Top Temperature ' + figdate)
ax.xaxis.set_major_locator(hrs3)
ax.xaxis.set_major_formatter(hrsfmt)
ax.autoscale_view()
ax.grid(True)
plt.savefig(figdir + '/temptop.png', dpi=100)
